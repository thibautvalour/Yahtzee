...
class CollectSampleExperiments():
    ...
    def __init__(self, n_games, n_dices, dice_max_value, n_boxes, model_dice_1, model_dice_2, model_box):
        ...
        self.history_model_dice_1 = [[] for _ in range(n_games)]
        self.history_model_dice_2 = [[] for _ in range(n_games)]
        self.history_model_box = [[] for _ in range(n_games)]

    def initialize(self):
        self.value_box = np.zeros(
            (self.n_games, self.box_size), dtype=np.float32)
        self.is_box_checked = np.zeros(
            (self.n_games, self.box_size), dtype=np.float32)
        self.dice = np.zeros(
            (self.n_games, self.n_dices*self.dice_max_value), dtype=np.float32)
    ...
    def available_moves(self):
        ...
        n_identical_dices = np.zeros((self.n_games, 6), dtype=np.float32)
        ...
        available_boxes[:, 8] = (n_identical_dices == 3).any(
            axis=1) and (n_identical_dices == 2).any(axis=1)

        # Small Straight
        available_boxes[:, 9] = (n_identical_dices[:, :3] > 0).all(axis=1) or (
            n_identical_dices[:, 1:4] > 0).all(axis=1) or (n_identical_dices[:, 2:] > 0).all(axis=1)

        # Large Straight
        available_boxes[:, 10] = (n_identical_dices[:, :3] > 0).all(
            axis=1) or (n_identical_dices[:, 2:] > 0).all(axis=1)
        ...
        # If the player has no available box
        available_boxes_mask[(available_boxes_mask == 0).all(
            axis=1)*(1-available_boxes[:, 12])] = 1-self.is_box_checked

        # First case : Joker
        # In this case, the boxes [Full House, Small Straight, Large Straight] are added to available_boxes
        yahtzee_indexes_joker = available_boxes_mask[available_boxes[:,
                                                                     12]*self.is_box_checked[:, 12]]*(self.value_box[:, 12] == 0)
        available_boxes[yahtzee_indexes_joker, 8:10] = 1

        # Second case : Yahtzee Bonus
        yahtzee_indexes_bonus = available_boxes_mask[available_boxes[:,
                                                                     12]*self.is_box_checked[:, 12]]*(self.value_box[:, 12] > 0)
